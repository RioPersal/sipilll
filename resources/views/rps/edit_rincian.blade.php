...
@section('judul','Edit Data Rincian SKS')
@section('content')

<div class="card">
    <form class="form-horizontal" form action="{{url ('/rps/{matkul:id}/edit_rincian_rps')}}" method="POST">
        <div class="card-body">
            @csrf
            <input type="hidden" name="id" value="{{ old('id', $rincian->id) }}">
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-sm-10">
                    <select name="sub_cpmk" id="sub_cpmk" class="form-control @error('sub_cpmk') is-invalid @enderror">
                        @foreach ($cpmk->where('id',$rincian->sub_cpmk) as $cpmkk)
                            <option value="{{ $cpmkk->id }}">({{ $cpmkk->kode_cpmk }}) {{ $cpmkk->ket_cpmk }}</option>
                        @endforeach
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <select name="bobot_cpmk" id="bobot_cpmk" class="form-control @error('bobot_cpmk') is-invalid @enderror">
                        <option value="{{ $rincian->asesmen }}">{{ $value_asesmen }}-</option>
                    </select>
                    @error('bobot_cpmk')
                    <div class="invalid-feedback">
                        {{ $message }}
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-sm-10">
                    <select name="kajian" class="form-control @error('kajian') is-invalid @enderror">
                        <option value="{{ $rincian->kajian }}">{{ $rincian->kajian }}</option>
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-sm-2">
                    <select name="time" class="form-control @error('time') is-invalid @enderror">
                        <option value="{{ $rincian->time }}">{{ $rincian->time }}</option>
                        @for ($i = 1; $i < $count+1; $i++)
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control @error('pengalaman') is-invalid  @enderror" name="pengalaman" value="{{ old('pengalaman', $rincian->pengalaman) }}" placeholder="Pengalaman Belajar Mahasiswa">
                    @error('pengalaman')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                @foreach ($media as $mediaa)
                    <div class="col-sm-2">
                        <select name="media[]" class="form-control @error('media') is-invalid @enderror">
                            <option value="{{ $mediaa }}">{{ $mediaa }}</option>
                            <option value="Diskusi">Diskusi</option>
                            <option value="Presentasi">Presentasi</option>
                            <option value="Praktek">Praktek</option>
                            <option value="Tugas Mandiri">Tugas Mandiri</option>
